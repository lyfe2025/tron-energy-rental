  ...
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="text-center mb-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">订单管理 - 选择网络</h1>
        <p class="text-gray-600">请选择要管理订单的网络</p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div
          ...
          :key="network.id"
          class="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 cursor-pointer border-2 border-transparent hover:border-indigo-500"
          ...
          <div class="p-6">
            ...
            <div class="mt-4 p-3 bg-indigo-50 rounded-md">
              <div class="flex items-center">
                ...
                <span class="text-sm text-indigo-800">查看该网络的所有订单</span>
              </div>
            </div>
            ...
            <div class="mt-6">
              <button
                @click.stop="goToOrderManagement(network)"
...
const handleNetworkClick = (network: Network) => {
  ...
  goToOrderManagement(network)
}

// 跳转到订单管理
const goToOrderManagement = (network: Network) => {
  if (!network.is_active) {
    error('该网络当前不可用')
    return
  }
...
const loadNetworks = async () => {
  try {
    loading.value = true
    console.log('🔍 [Orders NetworkSelection] 开始加载网络列表...')
    const response = await networkApi.getNetworks()
    console.log('📡 [Orders NetworkSelection] API响应:', response)

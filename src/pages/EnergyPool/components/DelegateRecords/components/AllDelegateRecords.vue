<template>
  <div class="all-delegate-records">
    <!-- 筛选器 -->
            ...
            <div>
              <div class="flex items-center space-x-2">
                <span class="font-medium text-gray-900">
                  {{ getLocalOperationTypeText(record) }}
                </span>
                <span
                  :class="[
...
} = useDelegateRecordsCommon(props) // 不传递方向参数

// 获取记录的方向（相对于当前账户）
const getRecordDirection = (record: DelegateRecord) => {
...
// 获取方向样式类
const getDirectionClass = (record: DelegateRecord) => {
...
// 获取方向文本
const getDirectionText = (record: DelegateRecord) => {
  const direction = getRecordDirection(record)
  return direction === 'in' ? '获得' : '出去'
}

  ...
  async handleFlashRentPayment(transaction: any, networkId: string): Promise<void> {
    try {
      ...
      if (!this.validateFlashRentTransaction(transaction)) {
        console.warn('Invalid flash rent transaction:', transaction.txID);
        return;
      }

      const { from: fromAddress, amount: trxAmount } = transaction;
      
      console.log(`Processing flash rent payment: ${transaction.txID}`, {
        from: fromAddress,
        amount: trxAmount,
        networkId: networkId
      });
      ...
      const orderService = await this.getOrderService();
      await orderService.createFlashRentOrder(fromAddress, trxAmount, networkId, transaction.txID);
      
      console.log(`Flash rent payment processed successfully: ${transaction.txID}`);
    } catch (error) {
      console.error(`Failed to process flash rent payment ${transaction.txID}:`, error);

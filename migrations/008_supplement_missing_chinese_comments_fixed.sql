-- 补充缺失的中文注释迁移文件（修复版）
-- Migration: 008_supplement_missing_chinese_comments_fixed.sql
-- Created: 2025-01-28
-- Description: 为所有缺失中文注释的数据库表字段添加完整的中文注释（基于实际存在的表结构）
...
COMMENT ON COLUMN agents.user_id IS '关联的用户ID，引用telegram_users表';
COMMENT ON COLUMN agents.agent_code IS '代理商代码，用于标识代理身份';
COMMENT ON COLUMN agents.commission_rate IS '代理商佣金比例（0-1之间的小数）';
COMMENT ON COLUMN agents.status IS '代理商状态：pending=待审核，active=活跃，inactive=非活跃，suspended=已暂停';
COMMENT ON COLUMN agents.total_earnings IS '代理商累计收益（TRX）';
COMMENT ON COLUMN agents.total_orders IS '代理商累计订单数量';
COMMENT ON COLUMN agents.total_customers IS '代理商累计客户数量';
COMMENT ON COLUMN agents.approved_at IS '代理商审核通过时间';
...
COMMENT ON COLUMN pricing_history.changed_by IS '操作者用户ID，记录谁执行了变更操作';
